...
class StrategyAgent(Agent):
    ...
    def run(self) -> str:
        ...
        lines = [
            ...
            "",
            "## Bottom line",
            f"- Training match rate: **{recon.get('match_rate_pct', 'n/a')}%** ({recon.get('compliant', 0)}/{recon.get('required_pairs', 0)} required pairs).",
            f"- Open certification gaps: **{recon.get('gap_count', 0)}**.",
            f"- Inbox items that block ops: **{len(inbox.get('blockers') or [])}** of {inbox.get('total', 0)}.",
        ...
        if recon.get("gap_count"):
            dec = Decision(statement="Require named owners to close all CRITICAL training gaps before Friday standup.", evidence_ids=ev_ids[:8], owner="People + department leads", due="Friday")
            decisions.append(dec)
            lines.append(f"- `{dec.id}` {dec.statement} (evidence: {', '.join(dec.evidence_ids) or 'none'})")
        if inbox.get("blockers"):
            dec = Decision(statement="Do not send vendor replies until the human approves drafts. Queue blockers first.", evidence_ids=[f.id for f in findings if f.source_tool == "triage_inbox"][:6], owner="Ops")
            decisions.append(dec)
        ...
        lines += ["", "## What this packet is not", "- Not a live Outlook / Graph integration.", "- Not proof of production deployment.", "- Claims above are bound to evidence IDs from this run's tools."]
        md = "\n".join(lines) + "\n"
        self.board.kv["briefing_md"] = md
        self.board.kv["decisions"] = [d.model_dump() for d in decisions]
        self.board.add_finding(Finding(agent=self.id, severity=Severity.INFO, title="Briefing drafted", evidence=f"{len(decisions)} decisions, {len(findings)} findings cited", source_tool="blackboard", source_ref=self.board.run_id))
        return md
